...
#if USE_BCF
                                memmove_s( szLine_i2ia, (long long)res*sizeof(szLine_i2ia[0]) + 1, p, res * sizeof( szLine_i2ia[0] ) ); /* move them to the start of the line */ /* djb-rwth: function replaced with its safe C11 variant */
#else
                                memmove(szLine_i2ia, p, res * sizeof(szLine_i2ia[0] ) ); /* move them to the start of the line */
#endif
                            }
#if USE_BCF
                            memcpy_s( szLine_i2ia + res, ((long long)res2 + 1)*sizeof(szNextLine_i2ia[0]) + 1, szNextLine_i2ia, ( (long long)res2 + 1 ) * sizeof( szNextLine_i2ia[0] ) ); /* djb-rwth: cast operator added; function replaced with its safe C11 variant */
#else
