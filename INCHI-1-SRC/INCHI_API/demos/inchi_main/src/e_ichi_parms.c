    ...
    *ulDisplTime = 0;
    ...
    if (bNameSuffix || bOutputPath)
    {
        ...
        if (bOutputPath && szOutputPath[0] && p)
        {
            ...
            {
#if USE_BCF
                strcat_s( szOutputPath, sizeof(szOutputPath) + 1, r + 1 ); /* djb-rwth: function replaced with its safe C11 variant */
#else
        ...
        for (i = 0; p && i < MAX_NUM_PATHS; i++)
        {
            ...
            if (!ip->path[i] || !ip->path[i][0])
            {
                ...
                {
#if USE_BCF
                    strcpy_s( sz, (long long)len + 1, p ); /* djb-rwth: function replaced with its safe C11 variant; cast operator added */
#else
                    strcpy( sz, p );
#endif
#if ( BUILD_WITH_AMI == 1 ) && ( OUTPUT_FILE_EXT == 1 )
...
#endif
#if USE_BCF
                    strcat_s( sz, (long long)len + 1, szNameSuffix ); /* djb-rwth: function replaced with its safe C11 variant; cast operator added */
#else
                    strcat( sz, szNameSuffix );
#endif
                    if (!pLastExt)
#if USE_BCF
                        strcat_s( sz, (long long)len + 1, ext[i] ); /* djb-rwth: function replaced with its safe C11 variant; cast operator added */
#else
                        strcat( sz, ext[i] );
#endif
                    ip->num_paths++;
