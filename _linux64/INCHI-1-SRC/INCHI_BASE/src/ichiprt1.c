...
static int str_LineStart( const char *tag, char *tag2, int val2, 
                          INCHI_IOSTREAM_STRING *strbuf, int ind );
static int str_LineEnd( const char *tag, int *bOverflow, 
                        INCHI_IOSTREAM_STRING *buf, 
                        int ind, int bPlainTextTags );

static int CleanOrigCoord( MOL_COORD szCoord, int delim );
static int WriteOrigCoord( int num_inp_atoms, MOL_COORD *szMolCoord, int *i, 
                           char *szBuf, int buf_len);
static int WriteOrigAtoms( CANON_GLOBALS *pCG, int num_inp_atoms, inp_ATOM *at, int *i, 
                           char *szBuf, int buf_len, STRUCT_DATA *sd);
static int WriteOrigBonds( CANON_GLOBALS *pCG, int num_inp_atoms, inp_ATOM *at, int *i, 
                           char *szBuf, int buf_len, 
                           STRUCT_DATA *sd);

static void GetSaveOptLetters(unsigned char save_opt_bits, char* let1, char* let2);

static int Output_RecordInfo( INCHI_IOSTREAM *out_file, int num_input_struct,
                              int bNoStructLabels, const char *szSdfLabel, 
                              const char *szSdfValue, long lSdfId, char *pLF, char *pTAB );
static int OutputINCHI_VersionAndKind( INCHI_IOSTREAM *out_file, 
                                       INCHI_IOSTREAM_STRING *strbuf,
                                       int bINChIOutputOptions,
                                       int is_beta, char *pLF, char *pTAB );
...
static int OutputINCHI_IsotopicLayer( CANON_GLOBALS *pCG, 
                                      INCHI_IOSTREAM *out_file, INCHI_IOSTREAM_STRING *strbuf,
                                      int *INCHI_basic_or_INCHI_reconnected,
                                      OutputINCHI_CtlData *io,
                                      char *pLF, char *pTAB );
...
static int OutputAUXINFO_PolymerInfo( CANON_GLOBALS *pCG,
                                      ...
                                      OutputINCHI_CtlData *io, 
                                      char *pLF, char *pTAB );
int psbond_min_num_compare( int *bond1, int* bond2 );

void HidePolymerRelatedInternals( INCHI_IOSTREAM *out );
int CountZzInFormula( char *s);
...
/*	
    ...
    (used to check for repeating (sub)layer(s) )			
*/
                  ...
                  unsigned char save_opt_bits)
{
    ...
    */
    ...
    int nAtomsAllComp;	/* v. 1.05 Total atoms in all components */
    ...
    io.ATOM_MODE = ((io.bAbcNumbers?CT_MODE_ABC_NUMBERS:0)
                    ...
                    | (io.bAbcNumbers && bCtPredecessors? CT_MODE_ABC_NUM_CLOSURES:0)
#endif
    ...
    */
    ...
    {
        bHasIsotopicAtoms[i]      = io.num_comp[i]                  =
        ...
        io.bRacemicStereo[i]      = io.bRelativeStereo[i]            =
        io.bIsotopicRacemicStereo[i] = io.bIsotopicRelativeStereo[i]    =
        io.bAtomEqu[i]            = io.bTautEqu[i]                   =
        io.bIsotopicAtomEqu[i]       = io.bIsotopicTautEqu[i]           =
        io.bInvStereo[i]             = io.bInvIsotopicStereo[i]         =
        io.bInvStereoOrigNumb[i]     = io.bInvIsotopicStereoOrigNumb[i] =
        io.bIgn_UU_Sp3[i]            = io.bIgn_UU_Sp2[i]                =
        io.bIgn_UU_Sp3_Iso[i]        = io.bIgn_UU_Sp2_Iso[i]            =
        io.bChargesRadVal[i]         = io.bOrigCoord[i]                 = 0;
    }

    /*	Find if it is isotopic */
    io.bIsotopic	= io.bTautomeric = io.bNonTautomeric = bTautomericAcid = 
    ...
    io.pINChISort = 
        io.pINChISortTautAndNonTaut[io.bTautomericOutputAllowed? TAUT_YES : TAUT_NON];
    is  = io.pINChISort;
    is2 = (io.bOutType== OUT_TN)? io.pINChISortTautAndNonTaut[TAUT_NON] : NULL;


    for ( i = 0, is2 = io.pINChISortTautAndNonTaut[TAUT_NON]; 
          i < io.num_components; 
          i++, is++, is2? is2++ : NULL ) 
    {
        ...
        io.bNonTautIsIdenticalToTaut = 
            io.bNonTautIsIdenticalToTaut && !CompINChITautVsNonTaut(is, is2, 1);
    ...
    for ( i = 0, is = io.pINChISort; i < io.num_components; i ++, is ++ )
    {
        ...
        for ( j = TAUT_NON; j < TAUT_NUM; j ++ ) 
        {
            ...
            if ( (pINChI = is->pINChI[jj]) && pINChI->nNumberOfAtoms > 0 ) 
            {
                ...
                if ( j == TAUT_NON )
                    nAtomsAllComp+= pINChI->nNumberOfAtoms;
    ...
    io.iCurTautMode = io.bOutType == OUT_N1? TAUT_NON:  /*  only non-taut									*/
                   ...
                   io.bOutType == OUT_TN? TAUT_YES:		/*  tautomeric if present otherwise non-tautomeric; */
                                             -1;		/*  separately output non-taut representations of 
                                                            tautomeric if present							*/
    ...
    */
    ...
    if ( INCHI_basic_or_INCHI_reconnected == INCHI_BAS ) 
        Output_RecordInfo( out_file, num_input_struct, bNoStructLabels, szSdfLabel, szSdfValue, lSdfId, pLF, pTAB );
    ...
    /* InChI output: atoms */
    intermediate_result	= OutputINCHI_MainLayerFormula( pCG, out_file, strbuf,  num_components2,
                                                        &INCHI_basic_or_INCHI_reconnected,
                                                        &io, pLF, pTAB );
...
repeat_INChI_output:
    ...
    */
         ...
         (bINChIOutputOptions & INCHI_OUT_EMBED_REC) && num_components2[INCHI_REC] ) 
    {
        ...
        nRet = OutputINChI1( pCG, strbuf, pINChISortTautAndNonTaut2, 
                             INCHI_REC, pOrigStruct,  0 /*bDisconnectedCoord*/, 
                             bOutputType, bINChIOutputOptions | INCHI_OUT_NO_AUX_INFO,
                             bAbcNumbers, bCtPredecessors, bNoStructLabels, 
                             num_components2, num_non_taut2, num_taut2,
                             out_file, log_file, num_input_struct,
                             szSdfLabel, szSdfValue, lSdfId, 
                             pSortPrintINChIFlags, save_opt_bits);
    ...
    /* Here! Intercept point: edit polymer layer if it is present */
    if ( pOrigStruct && pOrigStruct->polymer )
    {
        HidePolymerRelatedInternals( out_file );
    }
...
repeat_INChI_Aux_output:
        ...
        intermediate_result = OutputAUXINFO_OriginalNumbersAndEquivalenceClasses( pCG, 
                                                                                out_file, strbuf, 
                                                                                num_components2, 
                                                                                &io, pLF, pTAB );
        if ( intermediate_result != 0 ) 
        ...
        intermediate_result = OutputAUXINFO_TautomericGroupsEquivalence( pCG, 
                                                                            out_file, strbuf, 
                                                                            &io, pLF, pTAB );
        if ( intermediate_result != 0 ) 
            goto exit_function;
...
repeat_INChI_Aux_Iso_output:
        ...
        */
        ...
        intermediate_result = OutputAUXINFO_ChargesRadicalsAndUnusualValences( pCG, 
                                                                                out_file, strbuf, 
                                                                                &io, pLF, pTAB );
        if ( intermediate_result != 0 ) 
            goto exit_function;
        ...
        intermediate_result = OutputAUXINFO_ReversibilityInfo( pCG, out_file, strbuf, 
                                                                 pOrigStruct, &io, pLF, pTAB );
        if ( intermediate_result != 0 ) 
            goto exit_function;

        /* Output polymeric Aux Info */
        intermediate_result = OutputAUXINFO_PolymerInfo( pCG,out_file, strbuf,
                                                            pOrigStruct, &io, pLF, pTAB );
        if ( intermediate_result != 0 ) 
            goto exit_function;
        ...
        */
             ...
             num_components2[INCHI_REC] && !(bINChIOutputOptions & INCHI_OUT_NO_AUX_INFO) ) 
        {
            ...
            nRet = OutputINChI1( pCG, strbuf, pINChISortTautAndNonTaut2, 
                                 INCHI_REC, NULL, 0 /*bDisconnectedCoord*/, 
                                 bOutputType, INCHI_OUT_ONLY_AUX_INFO | bINChIOutputOptions,
                                 bAbcNumbers, bCtPredecessors, bNoStructLabels, 
                                 num_components2, num_non_taut2, num_taut2,
                                 out_file, log_file, num_input_struct,
                                 szSdfLabel, szSdfValue, lSdfId, 
...
*/
int str_LineStart( const char *tag,char *tag2, int val2, 
                   INCHI_IOSTREAM_STRING *buf, int ind )
{
    if ( ind < 0 ) 
    {
        /* plain text output */
...
*/
int str_LineEnd( const char *tag, int *bOverflow, 
                 INCHI_IOSTREAM_STRING *buf, 
                 int ind, int bPlainTextTags )
{
...
int WriteOrigAtoms( CANON_GLOBALS *pCG, 
                    int num_inp_atoms, inp_ATOM *at, int *i, 
                    char *szBuf, int buf_len, STRUCT_DATA *sd)
{
...
/* 
    Output structure (compound) header 
*/
int Output_RecordInfo( INCHI_IOSTREAM *out_file, int num_input_struct,
                        int bNoStructLabels, const char *szSdfLabel, 
                        const char *szSdfValue, long lSdfId,
                        char *pLF, char *pTAB )
{
    if ( bNoStructLabels )
        return 0;
    ...
    {
        inchi_ios_print_nodisplay( out_file, "%sStructure: %d.%s%s%s%s", pLF, num_input_struct,
                                 SDF_LBL_VAL(szSdfLabel, szSdfValue) );
        if ( lSdfId ) 
        {
            (out_file->s.nUsedLength)--;
            inchi_ios_print_nodisplay( out_file, ":%ld", lSdfId );
        }
        inchi_ios_print_nodisplay( out_file, "%s", pTAB );
    }
...
/* 
    Output InChI: main layer - formula, connections and hydrogens (incl. tautomeric info == mobile H) 
*/
                                      ...
                                      char *pLF, char *pTAB )
{
    ...
    if ( io->tot_len != io->tot_len2 ) { /* 2004-06-30: never output empty connection table */
                                    ...
                                    char *pLF, char *pTAB )
{
    ...
    if ( INCHI_SEGM_FILL == INChI_SegmentAction( io->sDifSegs[io->nCurINChISegment][DIFS_h_H_ATOMS] ) ) 
    {
        ...
        io->tot_len2 = str_H_atoms( io->pINChISort, strbuf, &io->bOverflow, io->bOutType,
                                ...
                                io->num_components, io->bUseMulipliers);
        if ( io->tot_len != io->tot_len2 ) { /* 2004-06-21: never output empty */
            io->tot_len = io->tot_len2;
                             ...
                             char *pLF, char *pTAB )
{
         ...
         INChI_SegmentAction( io->sDifSegs[io->nCurINChISegment][DIFS_s_STYPE] ) )
    {
        ...
        if ( io->nSegmAction = INChI_SegmentAction( io->sDifSegs[io->nCurINChISegment][DIFS_b_SBONDS] ) )
        {
            ...
            if ( INCHI_SEGM_FILL == io->nSegmAction ) 
            {
                io->tot_len = str_Sp2( io->pINChISort, 
                                   io->pINChISort2, 
                                   strbuf, 
                                   &io->bOverflow, 
                                   io->bOutType, 
        ...
        {
            if ( io->bPlainTextTags == 1 ) inchi_ios_print_nodisplay( out_file, "/" ); /* sp2 */
        }
        ...
        if ( io->nSegmAction = INChI_SegmentAction( io->sDifSegs[io->nCurINChISegment][DIFS_t_SATOMS] ) )
        {
            ...
            if ( INCHI_SEGM_FILL == io->nSegmAction ) 
            {
                io->tot_len = str_Sp3( io->pINChISort, io->pINChISort2, 
                                   strbuf, &io->bOverflow, io->bOutType, 
                                   io->TAUT_MODE, io->num_components, io->bRelRac,
                                   io->bSecondNonTautPass, io->bOmitRepetitions, io->bUseMulipliers);
                io->bNonTautNonIsoIdentifierNotEmpty += io->bSecondNonTautPass;
            }			
        ...
        if ( io->bPlainTextTags == 1 ) inchi_ios_print_nodisplay( out_file, "/" );  /* no abs, inv or racemic stereo */
    } 
    else
    {
                                 ...
                                 char *pLF, char *pTAB )
{
    ...
    if ( INChI_SegmentAction( io->sDifSegs[io->nCurINChISegment][DIFS_i_IATOMS] ) )
    {
        ...
        if ( io->nSegmAction = INChI_SegmentAction( io->sDifSegs[io->nCurINChISegment][DIFS_i_IATOMS] ) )
        {
            ...
            if ( INCHI_SEGM_FILL == io->nSegmAction )
            {
                io->tot_len2 = str_IsoAtoms( io->pINChISort, io->pINChISort2, 
                                         strbuf, &io->bOverflow, io->bOutType, io->TAUT_MODE, 
                                         io->num_components, io->bAbcNumbers,
             ...
             INChI_SegmentAction( io->sDifSegs[io->nCurINChISegment][DIFS_s_STYPE] ) )
        {
            ...
            if ( io->nSegmAction = INChI_SegmentAction( io->sDifSegs[io->nCurINChISegment][DIFS_b_SBONDS] ) )
            {
                ...
                if ( INCHI_SEGM_FILL == io->nSegmAction ) 
                {
                    io->tot_len = str_IsoSp2( io->pINChISort, io->pINChISort2, 
                                          strbuf, &io->bOverflow, io->bOutType, io->TAUT_MODE, io->num_components,
                                          io->bSecondNonTautPass, io->bOmitRepetitions, io->bUseMulipliers);
                    io->bNonTautIsoIdentifierNotEmpty += io->bSecondNonTautPass;
                }
            ...
            if ( io->nSegmAction = INChI_SegmentAction( io->sDifSegs[io->nCurINChISegment][DIFS_t_SATOMS] ) )
            {
                ...
                if ( INCHI_SEGM_FILL == io->nSegmAction ) 
                {
                    io->tot_len = str_IsoSp3( io->pINChISort, io->pINChISort2, 
                                          strbuf, &io->bOverflow, io->bOutType, 
                                          io->TAUT_MODE, io->num_components, io->bRelRac,
                                          io->bSecondNonTautPass, io->bOmitRepetitions, 
                                          io->bUseMulipliers);
         ...
         INCHI_SEGM_FILL == INChI_SegmentAction( io->sDifSegs[DIFL_F][DIFS_o_TRANSP] )) 
    {
        ...
        if ( 0 < bin_AuxTautTrans(io->pINChISort, io->pINChISort2, &nTrans_n, &nTrans_s, io->bOutType,  io->num_components) ) 
        {
            ...
            io->tot_len = str_AuxTautTrans( pCG, nTrans_n, nTrans_s, 
                                        strbuf, &io->bOverflow, 
            ...
            (*io->pSortPrintINChIFlags) |= 
                ( *INCHI_basic_or_INCHI_reconnected == INCHI_BAS )?  FLAG_SORT_PRINT_TRANSPOS_BAS :
...
{
    int i, j, b, k, err = 0;
    int tmp, a1=0, a2=0, a3=0, a4=0, n_used_stars=0, curr_star_num;
    int nunits2 = 0;
    int *cano_nums = NULL, *compnt_nums = NULL, *unum = NULL, *old_stars=NULL;
    OrigAtDataPolymerUnit *u=NULL;
    OrigAtDataPolymerUnit **units2=NULL;
    OrigAtDataPolymer *p = NULL;
    ...
    cano_nums	= inchi_calloc( pOrigStruct->num_atoms + 1, sizeof(int)  );
    if ( NULL	== cano_nums )							
    { 
        err = 1; 
        goto exitf; 
    }
    compnt_nums	= inchi_calloc( pOrigStruct->num_atoms + 1, sizeof(int)  );
    if ( NULL	== compnt_nums )						
    { 
        err = 2; 
        goto exitf; 
    }
    err = get_canonical_atom_numbers_and_component_numbers( pCG, strbuf, io, 
                                                            pOrigStruct->num_atoms, 
                                                            cano_nums, compnt_nums );
    if ( err != 0 )								
    { 
        err = 3; 
        goto exitf; 
    ...
    /* Make a working copy of polymer units data */
    
    units2 = inchi_calloc( p->n, sizeof(OrigAtDataPolymerUnit*) );
        /*	units2	is a copy of original polymer units (p->units) with atomic numbers
    ...
    old_stars	= inchi_calloc( pOrigStruct->polymer->n_star_atoms, sizeof(int)  );
    if ( NULL == old_stars)							
    { 
        err = 3; 
        goto exitf; 
    }
    for (i=0; i<pOrigStruct->polymer->n_star_atoms; i++)
        old_stars[i] = pOrigStruct->polymer->star_atoms[i];


    for (i=0; i<p->n; i++)
    {
        units2[i] = OrigAtDataPolymerUnit_CreateCopy( p->units[i] );
        if ( NULL == units2[i] )
        {
            err = 4; 
            goto exitf; 
    ...
    unum = inchi_calloc( p->n, sizeof(int) );
    ...
    err = OrigAtDataPolymer_PrepareWorkingSet( p, cano_nums, compnt_nums, units2, unum );
    ...
    for (i=0; i<p->n; i++)
    {
        ...
        /* Print unit-to-outside bonds or just outside-star-atoms (if phase-shiftable bonding) */
        if ( u->nb > 2 )						
        { 
        ...
        if ( u->nb == 2 && ( !u->closeable || !u->already_closed ) )
        {
            a1 = u->blist[0]; a2 = u->blist[1]; a3 = u->blist[2]; a4 = u->blist[3];
            if ( is_in_the_ilist( u->alist, a1, u->na ) )	{ tmp = a2; a2 = a1; a1 = tmp; }
            if ( is_in_the_ilist( u->alist, a3, u->na ) )	{ tmp = a4; a4 = a3; a3 = tmp; }
            if ( a1 < a3 )
                inchi_strbuf_printf( strbuf, "(%-d-%-d,%-d-%-d)",	a1, a2, a3, a4 );
            else
                inchi_strbuf_printf( strbuf, "(%-d-%-d,%-d-%-d)",	a3, a4, a1, a2 );
        }
        else if ( u->nb <= 2 && ( u->closeable || u->npsbonds > 0 ) )
        {	
            /* indicate phase shifted */
            
            /*	Get actual star atoms numbers from all-units pool
                NB: ordered according to already established in units2/unum		*/
            if ( u->star1 > 0 || u->star2 > 0 )
            {
                ...
                if ( u->star1 > 0 )
                {
                    curr_star_num = pOrigStruct->num_atoms - n_expl_H - p->n_star_atoms + n_used_stars + 1;
            ...
            /* if ( a1 > a2 )	{ tmp = a2; a2 = a1; a1 = tmp; } */
            inchi_strbuf_printf( strbuf, "(%-d,%-d-", a1,a2 );

            if ( u->closeable == CLOSING_SRU_DIRADICAL )
            {
                inchi_strbuf_printf( strbuf, "%-d)", u->star_partner1 );
            }
            else if ( u->closeable == CLOSING_SRU_HIGHER_ORDER_BOND )
            {
                /*a3 = u->psbonds[0][0]; a4 = u->psbonds[0][1]; */
                a3 = u->star_partner1;
                a4 = u->star_partner2;
                if ( a3 > a4 )	
                    { tmp = a4; a4 = a3; a3 = tmp; }
            ...
            else if  ( u->closeable == CLOSING_SRU_RING )
            {
                if ( u->npsbonds == 0 )
                {
                    /* last resort */
                ...
                {
                    /* Sort all psbonds in min-at-number order */
                    for (b=1; b<u->npsbonds; b++)
                    {
                        int *tmp = u->psbonds[b];
                        j = b - 1;
                        while ( j >= 0 &&	psbond_min_num_compare( u->psbonds[j], tmp ) > 0  )
                        {
                            u->psbonds[j+1] = u->psbonds[j];
                            j--;
                        }
                        u->psbonds[j+1] = tmp;
                    }
                    ...
                    {
                        a3 = u->psbonds[k][0]; a4 = u->psbonds[k][1]; 
                        /*if ( a3 > a4 )	{ tmp = a4; a4 = a3; a3 = tmp; }*/
                        inchi_strbuf_printf( strbuf, "%-d,%-d%-c", a3, a4, k==u->npsbonds-1?')':',' );
                    }
                }
            }
        ...
        if ( i < p->n-1 )	inchi_strbuf_printf( strbuf, ";" );
    }
...
exitf:
    if ( NULL != cano_nums )	inchi_free( cano_nums );
    if ( NULL != compnt_nums )	inchi_free( compnt_nums );
    if ( unum )					inchi_free( unum );
    if ( NULL != units2 )
    {
        for (i=0; i < nunits2; i++)
            OrigAtDataPolymerUnit_Free( units2[i] );
        inchi_free( units2 );
    }
    if (old_stars )
    {
        for (i=0; i<pOrigStruct->polymer->n_star_atoms; i++)
            pOrigStruct->polymer->star_atoms[i] = old_stars[i];
...
/* 
    Output AuxInfo: 
        reversibility info (to restore orig. structure)  
*/
...
void HidePolymerRelatedInternals( INCHI_IOSTREAM *out )
{
char *s = out->s.pStr, *buf=NULL;
char prev_layer_symbol = '0';
int i, j, ii, nzz, nzz1 = 0,	/* nzz2 = 0, */
    nslash = 0, ncopied = 0, 
    in_z = 0, skip = 0;

    nzz1 = CountZzInFormula( s );
    if ( nzz1 == 0 )	return;

    buf = inchi_calloc( out->s.nUsedLength + 1, sizeof(char) );

    nzz = nzz1;
    in_z = skip = 0;
    for (i=0; i<out->s.nUsedLength; i++)
    {
        int pre_eol = ( i == out->s.nUsedLength - 1 );
    ...
    inchi_ios_print_nodisplay( out, "%s", buf );
...
int CountZzInFormula( char *s)
{
    ...
    for (i=0; i< len; i++)
    {
        ...
        if ( nslash == 2 )
        {
            if ( s[i-1] != 'z' || s[i-2] != 'Z' )	break;
            for ( j = i-3; j>=0; j-- )
                if ( !isdigit( UCINT s[j]) ) 
                    break;
            nzz = strtol( s + j +1, NULL, 10);
        }
    }
    return nzz;
}
