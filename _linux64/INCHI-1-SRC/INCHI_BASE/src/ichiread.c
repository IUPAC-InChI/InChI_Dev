...
static int ParseSegmentPolymer( const char *str, int bMobileH, 
                                REM_PROTONS nNumProtons[], int nNumComponents[],
                                 ...
                                 OrigAtDataPolymer **ppPolymer,OrigAtDataV3000 **ppV3000 );
...
static int ReadInChICoord( INCHI_IOSTREAM *pInp, 
                           SEGM_LINE *pLine, 
                           int *pState,
                           INChI *pInpInChI[INCHI_NUM][TAUT_NUM],
                           int nNumComponents[INCHI_NUM][TAUT_NUM]);
...
static int ParseAuxSegmentReverseCRV( const char *str, 
                                      int bMobileH, 
                                      INChI *pInpInChI[], 
                                      int ppnNumComponents[], 
                                      int state);

static int ParseAuxSegmentReverseAtoms( const char *str, 
                                        int bMobileH, 
                                        INChI *pInpInChI[], 
                                        int ppnNumComponents[], 
                                        int state);

static int ParseAuxSegmentReverseBonds( const char *str, 
                                        int bMobileH, 
                                        INChI *pInpInChI[], 
                                        int ppnNumComponents[], 
                                        int state);
...
char * ParseSegmentReadDelimitedNumbers( const char *str, const char *pEnd, INT_ARRAY *numlist, char c_delim, char c_stop, int *ret );
#define SEG_END '/'
...
{                
    ...
    */
    ...
    if ( bInChI2Structure )
    {
        inchi_ios_init( pTmpOut, INCHI_IOSTREAM_TYPE_STRING, NULL );
        if ( pTmpOut->s.pStr )
            pRealOut = pTmpOut;
    }
    ...
    while( nReadStatus != RI_ERR_EOF ) 
    {
...
#endif
            /* polymer stuff added */
            if ( pInp->type == INCHI_IOSTREAM_TYPE_STRING )
            {
                int res;
        ...
        read_inchi_ok		=	end_of_data_reached && 
                                !nErr && 
        ...
        else
        {
            ...
            if ( pRealOut == pTmpOut )
            {
                ...
                inchi_ios_init( pRealOut, INCHI_IOSTREAM_TYPE_STRING, NULL );*/
            }
    ...
    do {
        ...
        case IST_MOBILE_H_POLYMER:       /* /z */
            ret = ParseSegmentPolymer(	pLine->str, bMobileH, 
                                        ...
                                        na_total, nb_total, ppPolymer, ppV3000 );
            if ( *ppPolymer )
                         ...
                         OrigAtDataV3000 **ppV3000 )
{
    ...
    const char *p=NULL, *q=NULL, *pStart=NULL, *pEnd=NULL;
    const char comma = ',', dot = '.', dash = '-', lt_par = '(', rt_par = ')', *p0;
    AT_NUMB num_atom;
    int curr_atom, type=-1, subtype=-1, conn=-1;
    INT_ARRAY alist;
    OrigAtDataPolymer	*pd	= NULL;

    if ( str[0] != 'z' )				
        return 0;

    if ( *ppPolymer )	
        OrigAtDataPolymer_Free( *ppPolymer );

    pd = *ppPolymer =	(OrigAtDataPolymer *) 
                        inchi_calloc( 1, sizeof(OrigAtDataPolymer) );
    (*ppPolymer)->star_atoms	=	NULL;
    ...
    pd->units = (OrigAtDataPolymerUnit**)			
    ...
    while( *pStart ) 
    {
        OrigAtDataPolymerUnit *unit=NULL;
        ...
        if ( alist.used )
        {		
            unit = OrigAtDataPolymerUnit_New( 4, 0,			/* maxatoms, maxbonds=0 now	*/
                                    iunit + 1, iunit + 1,	/* id,	label				*/
                                    type, subtype, conn,
                                    "",						/* smt						*/
                                    alist.used, &alist,		/* alist.used, &alist,		*/
                                    0, NULL,				/* blist.used, &blist,		*/
                                    0, NULL					/* npsbonds, int **psbonds	*/
                                   );
        ...
        if ( *p == lt_par )
        {
            ...
            while ( *(++p) )
            {
                if ( *p == '-' )	{ have = endgroups; break; }
                if ( *p == ',' )	{ have = stars; break; }
            }
            if ( have == stars )
            {
                while ( *(++p) )
                {
                    if ( *p == ',' )	{ have = stars_ring; break; }
                    if ( *p == '.' )	{ have = stars_bond; break; }
                    if ( *p == ')' )	{ have = stars_atom; break; }
                }
            }
            p = p0;
            if ( have == endgroups )
            {
                ...
                if ( res == 1 && numlist.used == 4 )
                {
                    pStart = p + 1;	
                    unit->nb = 2;
                    unit->blist =		inchi_calloc( 2*unit->nb, sizeof(int) );
                    if (!unit->blist )	{ ret = RI_ERR_ALLOC; IntArray_Free( &numlist ); goto exit_function; }
                    ...
                    continue;
                      ...
                      have == stars_atom	)
            {
                /* Read star atoms - phase shiftable bonds notation */
                ...
                if ( have == stars_ring	)
                {
                    ...
                    if ( res != 1 || numlist.used < 4 )	{ ret = RI_ERR_SYNTAX; IntArray_Free( &numlist ); goto exit_function; }
                    unit->closeable = CLOSING_SRU_RING;
                    unit->npsbonds = (numlist.used - 2)/2;
                }
                ...
                {
                    int num = inchi_strtol( (const char *) ++p, (const char **) &p, 10 );
                    if ( *p!= rt_par )					{ ret = RI_ERR_SYNTAX; IntArray_Free( &numlist ); goto exit_function; }
            ...
            unit->closeable = 1;
            unit->star1 = numlist.item[0];
            unit->star2 = numlist.item[1];
            unit->maxpsbonds = inchi_max( unit->maxpsbonds, unit->npsbonds );
            err	= imat_new( unit->maxpsbonds, 2, &(unit->psbonds) );
            for (ib=0; ib<unit->npsbonds; ib++)
            {
                unit->psbonds[ib][0] = numlist.item[ib*2 + 2 ];
                if ( numlist.used != 3 )
                    unit->psbonds[ib][1] = numlist.item[ib*2 + 3 ];
                else
                    unit->psbonds[ib][1] = unit->psbonds[ib][0];
            }
            if ( unit->npsbonds > 0 )
            {
                unit->closeable = 1;
    ...
    pd->really_do_phase_shift	=	1;
    pd->is_in_reconn			=	0;
    OrigAtDataPolymer_DebugTrace( pd );
    ret = pd->n;
...
char * ParseSegmentReadDelimitedNumbers( const char *str, const char *pEnd, INT_ARRAY *numlist, char c_delim, char c_stop, int *ret ) 
{
    ...
    *ret = 1;
    ...
    while( *pStart ) 
    {
        ...
        {
            num = (AT_NUMB)inchi_strtol( (const char *) p, (const char **) &p, 10 );
            {
                                   ...
                                   InpInChI *pOneInput)
{
    ...
    else 
    {
            ...
            ( pState==IST_MOBILE_H_POLYMER || !ip->bPolymers ) )
        {
            ...
            inchi_ios_eprint( pLog, "%s Skipping polymer compound in conversion of InChI to InChI\n", 
...
int ret, tmp;
    ...
    ret = OutputInChIAsRequested( pCG, 
                                  pOut, 
                                  pLog, 
                                  ip, 
                                  sd, 
...
int bINChIOutputOptions = 
    ...
    ret = DetectInpInchiCreationOptions( pOneInput, 
                                         &bHasSomeReconnected, 
                                         &bHasMetal,
                                         &bHasSomeFixedH, 
                                         &nModeFlagsStereo, 
    ...
    ret = AllInchiToStructure( ic, pCG, ip, sd, num_inp, 
                               *pszCurHdr, &srm, bHasSomeFixedH, 
                               pStruct, pOneInput);
    ...
    {
        ret = AddProtonAndIsoHBalanceToMobHStruct( ic, pCG, 
                                                   ip, 
                                                   sd, 
                                                   num_inp, 
                                                   bHasSomeFixedH, 
    ...
    ret = CompareAllOrigInchiToRevInChI(pStruct, 
                                        pOneInput, 
                                        bHasSomeFixedH, 
                                        num_inp, 
                                        *pszCurHdr);
    ...
    ret = CompareAllDisconnectedOrigInchiToRevInChI(pStruct, 
                                                    pOneInput, 
                                                    bHasSomeFixedH,
                                                    num_inp, 
                                                    *pszCurHdr);
    ...
    ret = DisplayStructureComponents( pCG, 
                                      ip, 
                                      sd, 
                                      num_inp, 
                                      *pszCurHdr,
    ...
    ret = MergeStructureComponents( ip, 
                                    sd, 
                                    num_inp, 
                                    *pszCurHdr, 
                                    &srm, 
...
int	 i, j, elindex, ret = 0, nheavy = 0, 
     ...
     slen = 0, ninsert= 0, kinsert, lead_pos, nc, ntimes;
char *p = NULL, *pz = NULL, *pz2 = NULL, *pr = NULL, *pend = NULL, *q = NULL;
...
int *insert_pos = NULL;	/* inserts go before insert_pos[k] */
    ...
    while ( isspace( UCINT s[is->s.nUsedLength-1] ) )
    {
        s[--is->s.nUsedLength] = '\0';
    }
    i_last_sym = is->s.nUsedLength - 1;
    
    /*if ( is->s.pStr[i_last_sym-1] == '\n' )
        i_last_sym--;
    */
    ...
    while ( p != pend )
    {
        if ( isdigit( *p ) )  
        {
            ntimes = (int) inchi_strtol( p, (const char **) &q, 10 );
            p = q;
        } 
        else 
        {
        ...
        if ( *p && isdigit( UCINT *p ) ) 
        {
            elindex = (int) inchi_strtol( p, (const char **) &q, 10 );
            p = q;
        } 
        else 
            elindex = 1;
    ...
    insert_pos = inchi_calloc( nheavy + 32, sizeof(int) );
    if ( !insert_pos ) 
        { ret = -2; goto endf; }
    
    ninsert = 0;
    if ( pend )		insert_pos[ninsert] = (int) ( pend - s );
    else			insert_pos[ninsert] = i_last_sym;
    ninsert++;
    ...
    if ( pend )
        ...
        zlen = strlen(pz);
    tmpstr = inchi_calloc( zlen + 32, sizeof(char) );
    if ( !tmpstr) 
        { ret = -2; goto endf; }
    ...
    slen = strlen ( s );
    edited_s  = inchi_calloc( slen*100 + 32*10*ninsert, sizeof(char) ); /* high reservation */
    if ( !edited_s ) 
        { ret = -2; goto endf; }
    ...
    /*inchi_free(is->s.pStr);
    is->s.pStr = NULL;
    is->s.nUsedLength = 0;*/
    /*inchi_strbuf_reset( &is->s );*/
    inchi_strbuf_close( &is->s );
    inchi_ios_print( is, "%s\n", edited_s );


    
endf:
    if ( edited_s )		inchi_free( edited_s );
    if ( tmpstr)		inchi_free( tmpstr );
    if ( insert_pos )	inchi_free( insert_pos );
