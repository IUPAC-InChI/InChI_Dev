...
int SetForbiddenEdges( BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms, 
                       int forbidden_mask, int nebend, int *ebend )
{
...
{   
    ...
    for ( j=0; j<nebend; j+=2 )
    {
        ...
        for ( i = 0; i < num_atoms; i++ ) 
        {
            if ( at[i].orig_at_number == ia1 )			i1 = i;
            else if ( at[i].orig_at_number == ia2 )		i2 = i;
            if ( i1 > 0 && i2 > 0 )						break;
        }
            ...
            if ( *pbTautFlags & TG_FLAG_TEST_TAUT__SALTS ) 
            {
                ...
                if ( *pbTautFlags & TG_FLAG_TEST_TAUT2_SALTS ) 
                {
                    ...
                    { 
                        ...
                        CorrectFixing_NH_NH_Bonds( pBNS, at, num_atoms );
#endif
                        salt_found = MarkSaltChargeGroups( pCG, 
                                                           at, 
                                                           num_atoms, s_group_info,
                                                           t_group_info, 
                                                           c_group_info, pBNS, pBD );
#if ( RESET_EDGE_FORBIDDEN_MASK == 0 )
                        pBNS->edge_forbidden_mask &= ~BNS_EDGE_FORBIDDEN_TEMP;
#endif
                        if ( salt_found < 0 ) {
                    ...
                    if ( salt_step /*||
                        ...
                        CorrectFixing_NH_NH_Bonds( pBNS, at, num_atoms );
#endif
                        salt_found = MarkSaltChargeGroups2( pCG, 
                                                            at, num_atoms, 
                                                            s_group_info,
                                                            t_group_info, 
                                                            c_group_info, 
...
int bExistsAnyAltPath( CANON_GLOBALS *pCG, BN_STRUCT *pBNS, BN_DATA *pBD, 
                       inp_ATOM *at, int num_atoms,
...
*/
int bExistsAltPath( CANON_GLOBALS *pCG, BN_STRUCT *pBNS, BN_DATA *pBD, BN_AATG *pAATG, 
                    inp_ATOM *at, int num_atoms,
