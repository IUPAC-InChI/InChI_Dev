                                     ...
                                     int *num_changes )  
{
    ...
    */
    ...
    enum elems { dNone, dCl=17,dBr=35,dI=53,dAt=85, dO=8,
                          ...
                          INCHI_MODE *bTautFlagsDone )
{
    ...
    if ( nNumImplH )
        *nNumImplH = num_changes ? num_impl_H+1 
                                 : 0;
...
#define NUM_SEGM 20
    ...
    ave_bond_len = GetMinDistDistribution( at, 
                                           num_at, 
                                           iat, 
                                           iat_H, 
                                           bInAllComponents, 
                                ...
                                int bProcessOldCompNumbers )
{
    ...
    */
    ...
    if ( !( nNewCompNumber = (AT_NUMB *) 
                                inchi_calloc( num_at, sizeof(nNewCompNumber[0]) ) ) ||
         /* for non-recursive DFS only: */
         !( nPrevAtom      = (AT_NUMB *) 
                                inchi_calloc( num_at, sizeof(nPrevAtom[0]) ) ) ||
         !( iNeigh         = (S_CHAR  *) 
                                inchi_calloc( num_at, sizeof(iNeigh[0]) ) )) 
    {
        goto exit_function;
    }
...
int i, j, iat, nbr, jat, nj, degree, nat, err=0;
    ...
    err = 1;
    if ( ! (sg->orig2node = (int *) inchi_calloc( nat+1, sizeof( int ) )) )		goto exitf;	
    if ( ! (sg->nodes	  = (int *) inchi_calloc( nnodes, sizeof( int ) )) )	goto exitf;	
    if ( ! (sg->degrees	  = (int *) inchi_calloc( nnodes, sizeof( int ) )) )	goto exitf;	
    ...
    sg->nnodes = 0;
    for (i=0;i<nnodes;i++)		sg->nodes[ sg->nnodes++ ]		=	 nodes[i];
    
    for (i=0;i<=nat;i++)	sg->orig2node[ i ]			  =	-1;
    for (i=0;i<nnodes;i++)	sg->orig2node[ sg->nodes[i] ] =	 i;

    sg->adj	=		( subgraf_edge **) inchi_calloc( nnodes, sizeof( subgraf_edge * ) );
    if ( !sg->adj)	goto exitf;	
...
int i;
    if ( !sg )				return;
    if ( sg->nodes )		inchi_free ( sg->nodes );
    if ( sg->degrees )		inchi_free ( sg->degrees );
    if ( sg->orig2node)		inchi_free ( sg->orig2node );
    if ( sg->adj )
    {
        for (i=0; i<sg->nnodes; i++)	
            if ( sg->adj[i] )	
...
int p, q;
    ...
    for(p=0;p<sg->nnodes;p++)
    {
        ITRACE_("\tNode #%-d (atno: curr %-d, orig num %-d) ::: Neighbors (node, curr, orig) : ", 
                p, sg->nodes[p]-1, sg->nodes[p]);
        for (q=0; q<sg->degrees[p]; q++)
        {
            int nbr = sg->adj[p][q].nbr;
            ITRACE_("(%-d/%-d/%-d)  ", nbr, sg->nodes[nbr]-1, sg->nodes[nbr]);
        }
        ITRACE_("\n");
    }
    ITRACE_("\n* End Subgraf\n*********************************************************************\n");
...
subgraf_pathfinder * subgraf_pathfinder_new( subgraf *sg, ORIG_ATOM_DATA *orig_inp_data,
                                             int start, int end )
{
...
void subgraf_pathfinder_run( subgraf_pathfinder *spf, int *nbonds, int **bonds )
{
int j, k, node, node0;
    ...
    for ( j=0; j<spf->sg->degrees[node0]; j++ )
    {
        ...
        if ( node == spf->end )
        {
            ...
            {
                ITRACE_("%-d ", spf->sg->nodes[ spf->seen[k] ]);
            }
    ...
    for ( j=0; j<spf->sg->degrees[node0]; j++ )
    {
        ...
        spf->seen[ spf->nseen++ ] = node;	
        subgraf_pathfinder_run( spf, nbonds, bonds);
        spf->seen[ spf->nseen - 1 ] = 0;	
        spf->nseen--;	
    }
