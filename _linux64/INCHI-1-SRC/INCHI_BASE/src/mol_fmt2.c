...
/*	
    Read n chars and find where they are 
    terminated with space or trailing 0  
*/
...
char   *p, *q = NULL;
    ...
    if ( pHdr ) 
    {
        if ( !memicmp( pHdr->molname, sStruct, sizeof(sStruct)-1 ) ) 
        {
...
char szNumber[32];
    ...
    else if ( inp_file->type == INCHI_IOSTREAM_TYPE_STRING )
    {
        /* TODO: add code here. */
        ;
    }
...
double x_coeff, y_coeff, z_coeff, coeff=1.0, average_bond_length;
    ...
    *scaler = coeff;
    ...
    for ( i = 0; i < mfdata->ctab.n_bonds; i ++ ) 
    {
        ...
        double dx = mfdata->ctab.atoms[a1].fx-mfdata->ctab.atoms[a2].fx;
        double dy = mfdata->ctab.atoms[a1].fy-mfdata->ctab.atoms[a2].fy;
        double dz = mfdata->ctab.atoms[a1].fz-mfdata->ctab.atoms[a2].fz;
