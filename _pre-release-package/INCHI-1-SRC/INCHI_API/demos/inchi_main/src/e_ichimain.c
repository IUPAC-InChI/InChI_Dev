...
void dump_inchi_Atom( inchi_Atom *at, int iat, INCHI_IOSTREAM *pout );

void dump_inchi_Atom( inchi_Atom *at, int iat, INCHI_IOSTREAM *pout )
{
    int i;

    inchi_ios_eprint( pout, 
                      "\n%s%-d n_bonds=%-d nbr={",
                      at->elname, iat, at->num_bonds);
    
    for (i=0; i<at->num_bonds-1; i++)
        inchi_ios_eprint( pout, "%-d, ", at->neighbor[i]);
    
    if ( at->num_bonds>0 )
        inchi_ios_eprint( pout, "%-d} b_typ={", at->neighbor[at->num_bonds-1] );
    else
        inchi_ios_eprint( pout, "} b_typ={" );
    for (i=0; i<at->num_bonds-1; i++)
        inchi_ios_eprint( pout, "%-d, ", at->bond_type[i]);
    
    if ( at->num_bonds>0 )
        inchi_ios_eprint( pout, "%-d} b_ster={", at->bond_type[at->num_bonds-1] );
    else
        inchi_ios_eprint( pout, "} b_ster={" );
    for (i=0; i<at->num_bonds-1; i++)
        inchi_ios_eprint( pout, "%-d, ", at->bond_stereo[i]);
    
    if ( at->num_bonds>0 )
        inchi_ios_eprint( pout, "%-d} chg=%-d, rad=%-d, num_iso_H={%-d,%-d,%-d,%-d} iso_mass=%-d XYZ={%-f, %-f, %-f}", 
                      at->bond_stereo[at->num_bonds-1],
                      at->charge,
                      at->radical,
                      at->num_iso_H[0], at->num_iso_H[1], at->num_iso_H[2], at->num_iso_H[3],
                      at->isotopic_mass,
                      at->x, at->y, at->z);
    else
        inchi_ios_eprint( pout, "} chg=%-d, rad=%-d, num_iso_H={%-d,%-d,%-d,%-d} iso_mass=%-d XYZ={%-f, %-f, %-f}", 
                      at->charge,
                      at->radical,
                      at->num_iso_H[0], at->num_iso_H[1], at->num_iso_H[2], at->num_iso_H[3],
                      at->isotopic_mass,
...
void dump_inchi_Input( inchi_Input *pinp, INCHI_IOSTREAM *pout )
{
    ...
    inchi_ios_eprint( pout, "***** DUMP OF DATA STRUCTURE: inchi_Input *****");

    inchi_ios_eprint( pout, 
                      "\nnum_atoms=%-d num_stereo0D=%-d",
                      pinp->num_atoms, pinp->num_stereo0D);

    for (i=0; i<pinp->num_atoms; i++)
        dump_inchi_Atom( &(pinp->atom[i]), i, pout );

    inchi_ios_eprint( pout, "\n"); 
    inchi_ios_eprint( pout, "***** END OF DUMP *****\n");
}
