...
const char *platform="Linux";
    ...
    buffer = calloc( MOLBUFSIZE, sizeof(char) );
    if ( !buffer )
    {
    ...
    while ( get_next_molfile_as_text( sdfile, buffer, MOLBUFSIZE ) > 0 )
    {
        ...
        if (round_trip)
        {
            ...
            saved_inchi = realloc(saved_inchi, strlen(inchi) + 1);
...
cleanup:
    ...
    fprintf(stderr, "\nFinished; processed %-ld molecules with %-ld errors", 
                    index, nerr);
    if (round_trip)
    {
        fprintf( stderr, 
                 "\nRound trip problems occurred %-ld times (%-ld failures, %-ld mismatches)", 
                 nboom_rtrip + nmismatch_rtrip, 
                 nboom_rtrip, nmismatch_rtrip);
    }
    fprintf(stderr, ".\n");
...
{
        fprintf(stderr, "Usage: test_ixa <sd filename> <output filename> [/option[ /option...]]\n\n\n");
