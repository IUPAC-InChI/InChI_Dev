...
int MarkSaltChargeGroups2( CANON_GLOBALS *pCG, inp_ATOM *at, int num_atoms, S_GROUP_INFO *s_group_info,
...
bFound2Pairs:
        ...
        if ( nNumMarkedCandidates  ) {
            ...
            ret = RegisterEndPoints( pCG, t_group_info, EndPoint, nNumMarkedCandidates, at, num_atoms, c_group_info, pBNS );
...
int MergeSaltTautGroups( CANON_GLOBALS *pCG, inp_ATOM *at, int num_atoms, 
                         S_GROUP_INFO *s_group_info,
                         T_GROUP_INFO *t_group_info, C_GROUP_INFO *c_group_info,
                         struct BalancedNetworkStructure *pBNS )
{
...
*/
int MarkTautomerGroups( CANON_GLOBALS *pCG, inp_ATOM *at, int num_atoms, T_GROUP_INFO *t_group_info, C_GROUP_INFO *c_group_info
                        , struct BalancedNetworkStructure *pBNS, struct BalancedNetworkData *pBD )
{
                        ...
                        for ( m = 0; m < at[i2].valence; m ++ ) {
                            ...
                            */
                            ...
                            if ( at[i1].nNumAtInRingSystem >= 7 ) {
                                ret = nGet14TautIn7MembAltRing( pCG, at, i1, j, k, m, nDfsPathPos,
                                                              DfsPath,  nMaxLenDfsPath,
                                                              EndPoint, sizeof(EndPoint)/sizeof(EndPoint[0]),
                                                              BondPos, sizeof(BondPos)/sizeof(BondPos[0]),
                                                              &nNumEndPoints, &nNumBondPos, 
...
int SortTautomerGroupsAndEndpoints( CANON_GLOBALS *pCG, T_GROUP_INFO *t_group_info, int num_atoms, int num_at_tg, AT_RANK *nRank )
{
    ...
    for ( i = 0; i < num_t_groups; i ++ ) {
        ...
        insertions_sort( pn_tRankForSort, nEndpointAtomNumber, nNumEndpoints, sizeof(nEndpointAtomNumber[0]), CompRankTautomer);
    }
    ...
    if ( t_group_info->num_t_groups > 1 ) {
        ...
        insertions_sort( pn_tRankForSort, t_group_info->tGroupNumber, num_t_groups,
