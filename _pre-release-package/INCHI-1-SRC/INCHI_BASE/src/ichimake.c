...
int CompareIcr( ICR *picr1, ICR *picr2, 
                INCHI_MODE *pin1, INCHI_MODE *pin2, 
                INCHI_MODE mask )
{
...
/* */
    ...
    if ( orig_inp_data && orig_inp_data->polymer && orig_inp_data->polymer->n > 0 && orig_inp_data->polymer->valid )
    {
        ...
        for (j=0; j<orig_inp_data->polymer->n; j++)
            if ( orig_inp_data->polymer->units[j]->already_closed == 1 )
                nebend++;
        if ( nebend )
        {
            nebend*= 2;
            ...
            for (j=0; j<orig_inp_data->polymer->n; j++)
            {
                if ( orig_inp_data->polymer->units[j]->already_closed == 1 )
                {
                    ebend[jj]	=	orig_inp_data->polymer->units[j]->star_partner1;
                    ebend[jj+1] =	orig_inp_data->polymer->units[j]->star_partner2;
                    jj+=2;
                }
            }
        }
    ...
    */
    if ( out_norm_data[TAUT_YES]->at && at[TAUT_YES] ) 
    {
        ...
        */
        ...
        ret = set_stereo_parity( pCG, 
                                 out_norm_data[TAUT_YES]->at, at[TAUT_YES], 
    ...
    else if ( out_norm_data[TAUT_NON]->at && at[TAUT_NON] ) 
    {
        ...
        /* set stereo parities to at[]; nUserMode: accept alt. stereo bonds, min ring size */
        ret = set_stereo_parity( pCG, 
                                 out_norm_data[TAUT_NON]->at, at[TAUT_NON], num_atoms, num_removed_H,
...
/***************************************************************************************/
int FillOutCanonInfAtom( struct tagCANON_GLOBALS *pCG, 
                         inp_ATOM *norm_at, INF_ATOM_DATA *inf_norm_at_data, 
                         int init_num_at, int bIsotopic,
                         INChI *pINChI, INChI_Aux *pINChI_Aux, 
                         int bAbcNumbers, INCHI_MODE nMode)
{
...
/***************************************************************************************/
int FillOutOneCanonInfAtom( struct tagCANON_GLOBALS *pCG, 
                            inp_ATOM *inp_norm_at, INF_ATOM_DATA *inf_norm_at_data,
                            AT_NUMB *pStereoFlags, int init_num_at, 
                            int offset, int offset_H, int bIsotopic,
                            INChI *pINChI, INChI_Aux *pINChI_Aux, 
                            int bAbcNumbers, INCHI_MODE nMode)
{
    int          i, j, m, n, num_stereo, k, c, ret, len_str, len, atw, num_err;
...
int FillOutInfAtom( struct tagCANON_GLOBALS *pCG,  
                    inp_ATOM *norm_at, INF_ATOM_DATA *inf_norm_at_data, int init_num_at, int num_removed_H,
...
int FillOutCompositeCanonInfAtom( struct tagCANON_GLOBALS *pCG, 
                                  COMP_ATOM_DATA *composite_norm_data, INF_ATOM_DATA *inf_norm_at_data,
