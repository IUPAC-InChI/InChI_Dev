    ...
    *ulDisplTime  = 0;
    ...
    for ( i = 1; i < argc; i ++ ) 
    {
        ...
        if ( is_gui && INCHI_OPTION_PREFX == argv[i][0] && INCHI_OPTION_PREFX != argv[i][1] ) 
        {
            ...
            else if ( !stricmp( pArg, "OUTERRINCHI" ) ) 
            {    
...
#endif
            else if ( !stricmp( pArg, "MISMATCHISERROR" ) ) 
            {    
        ...
        else if ( (bVer1Options & 1) && INCHI_OPTION_PREFX == argv[i][0] && argv[i][1] ) 
        {
            ...
            else if ( !stricmp( pArg, "LargeMolecules" ) )  
            {
                 bLargeMolecules = 1;
            } 
            else if ( !stricmp( pArg, "Polymers" ) )  
            {
                 bPolymers = 1;
            } 
            /* InChIKey/InChI hash */
            ...
            else if ( !stricmp( pArg, "ONLYRECMET" ) ) {  /* do not disconnect metals */
                bDisconnectCoord = 0;
            } 
            else if ( !stricmp( pArg, "ONLYRECSALT" ) ) {  /* do not disconnect salts */
                bDisconnectSalts = 0;
            } 
            else if ( !memicmp( pArg, "MOVEPOS:", 8 ) ) {   /* added -- 2010-03-01 DT */
                bMovePositiveCharges = (0 != strtol(pArg+8, NULL, 10));
            } 
            else if ( !memicmp( pArg, "RSB:", 4 )) {
                mdbr = (int)strtol(pArg+4, NULL, 10);
