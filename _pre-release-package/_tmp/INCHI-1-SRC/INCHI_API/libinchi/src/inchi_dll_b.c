...
MakeINCHIFromMolfileText( const char *moltext, 
                          char *szOptions, 
                          inchi_Output *result )
{
...
INCHI_IOSTREAM_STRING *strbuf = &temp_string_container;
    ...
    retcode  = PrepareToMakeINCHI( sd, ip,
    ...
    retcode = GetOneStructure( &ic, sd,ip, szTitle, inp_file, plog, pout, pprb,
    ...
    else
    {
        ...
        if ( output_error_inchi )
        {
            ...
            if ( !result->szInChI )
            {
                ...
                result->szInChI = inchi_malloc( 12*sizeof(char));
...
ret:
    retcode2  = PostMakeINCHICleanup( &CG, 
                                      sd, ip,
                                      orig_inp_data, prep_inp_data,
                                      pINChI, pINChI_Aux,
...
#if 0
    /*if ( result->szLog && plog->s.pStr && plog->s.pStr[0] )&*/
    if ( plog->s.pStr && plog->s.pStr[0] )
        result->szLog = plog->s.pStr;
    plog->s.pStr = NULL;
...
char szSdfDataValue[MAX_SDF_VALUE+1];
    ...
    if ( options )
        quasi_options = (char*) malloc( strlen(options) + 1 );
    if ( quasi_options )
    {
    ...
    inchi_ios_init(pout, INCHI_IOSTREAM_TYPE_STRING, NULL);
    inchi_ios_init(plog, INCHI_IOSTREAM_TYPE_STRING, NULL);
    inchi_ios_init(pprb, INCHI_IOSTREAM_TYPE_STRING, NULL);
